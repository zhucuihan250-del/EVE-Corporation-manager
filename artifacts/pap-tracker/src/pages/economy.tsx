...
export function Economy() {
  ...
  return (
    <div className="p-6 space-y-6 overflow-auto">
      ...
      <Card>
        <CardHeader><CardTitle className="flex items-center gap-2"><BrainCircuit className="h-5 w-5 text-primary" />{tr("AI 快速增收行动方案", "AI rapid-income action plan")}</CardTitle><CardDescription>{data?.analysis ? `${data.analysis.summary} · ${tr("生成于", "Generated")} ${new Date(data.analysis.generatedAt).toLocaleString()}` : tr("同步钱包后生成具体、可执行、见效快的建议。", "Sync the wallet, then generate specific, feasible actions with a short time to impact.")}</CardDescription></CardHeader>
        <CardContent className="space-y-4">{!data?.analysis ? <p className="text-sm text-muted-foreground">{tr("暂无分析。", "No analysis yet.")}</p> : data.analysis.actions.map((action, index) => <div key={`${action.title}-${index}`} className="rounded-md border border-border/50 p-4 space-y-3"><div className="flex flex-wrap justify-between gap-2"><div className="font-semibold">{index + 1}. {action.title}</div><div className="flex gap-2"><Badge variant="outline">{tr(`${action.timeToImpactDays} 天见效`, `${action.timeToImpactDays} days to impact`)}</Badge><Badge>{tr(`月增收 ${formatIsk(action.expectedMonthlyGain)}`, `Monthly gain ${formatIsk(action.expectedMonthlyGain)}`)}</Badge></div></div><p className="text-sm text-muted-foreground">{action.evidence}</p><ol className="list-decimal pl-5 text-sm space-y-1">{action.steps.map((step) => <li key={step}>{step}</li>)}</ol><div className="grid gap-2 text-xs md:grid-cols-2"><div><span className="text-muted-foreground">{tr("责任人：", "Owner: ")}</span>{action.owner}</div><div><span className="text-muted-foreground">{tr("启动费用：", "Startup cost: ")}</span>{formatIsk(action.startupCost)}</div><div><span className="text-muted-foreground">KPI：</span>{action.kpi}</div><div><span className="text-muted-foreground">{tr("风险：", "Risk: ")}</span>{action.risk}</div></div><div className="space-y-1"><div className="flex justify-between text-xs text-muted-foreground"><span>{tr("置信度", "Confidence")}</span><span>{Math.round(action.confidence * 100)}%</span></div><Progress value={action.confidence * 100} /></div></div>)}</CardContent>
      </Card>
    </div>
  );
}
